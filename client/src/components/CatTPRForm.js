...
function CatTPRForm() {
  ...
  const handleFetch = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:3005/cat?temperature=${temperature}&pulse=${pulse}&respiration=${respiration}&petId=${petId}`, {
        method: 'GET',
      });
